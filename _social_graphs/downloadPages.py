    ...
    for j in range(10,12):
        ...
        for k in range(days):
            for l in range(10):
                ...
                cmd = "curl -u username:token -X GET 'https://api.github.com/search/repositories?q=game+created:%s-%s-%s&per_page=100&page=%s' > %s_%s_%s_%s.json" % (year_start,month,day, l+1, year_start,month,day,l+1)
                print(cmd)
                results = subprocess.run(cmd, shell=True, universal_newlines=True, check=True)
